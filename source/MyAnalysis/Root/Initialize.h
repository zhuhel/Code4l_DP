...
EL::StatusCode MyxAODAnalysis :: initialize ()
{
  ...
  //InitStrVec(STEP_cut,"xAOD,BadBatMan,FourMore,Quad,Kine,nCalo,dR4l,JPsiVeto,EleLoose,IP,Iso,OnShell,Filter,TwoJet,TwoSigJet,Trigger,dEtaJJ,MJJ,Cleaning");
  ...
  Info("initialize()", "Number of events = %lli", m_event->getEntries() );
    ...
    const char* GRLFilePath_2018 = "GoodRunsLists/data18_13TeV/20181111/data18_13TeV.periodAllYear_DetStatus-v102-pro22-04_Unknown_PHYS_StandardGRL_All_Good_25ns_Triggerno17e33prim.xml";
    const char* fullGRLFilePath_2018 = gSystem->ExpandPathName (GRLFilePath_2018);
    ...
    vecStringGRL.push_back(fullGRLFilePath_2018);
   ...
   // string PileConfigFile = "dev/PileupReweighting/mc15c_v2_defaults.NotRecommended.prw.root";
    string PileConfigFile = PathResolverFindCalibFile("MyAnalysis/mc16e.prw.root");
    ...
    Tree["sumweight"]=new TTree("sumweight","suw");
    Tree["sumweight"]->SetDirectory(file1);
    TreeDouVar["sumweight"]["Value"]=sumOfWeights;
    Tree["sumweight"]->Branch("sumweight", &TreeDouVar["sumweight"]["Value"],"sumweight/D");
    Tree["sumweight"]->Fill();
    MyInitTree2("eeee,mmmm,eemm,incl","weight,error",file1);
...
void MyxAODAnalysis :: MyInitTree2(string CHN, string BR, TFile *file1){
    ...
    for(int i=0; i<(int)channel.size();i++){
        name=channel[i];
        name1=name+"t";
        Tree[name.c_str()]=new TTree(name.c_str(),name1.c_str());
        Tree[name.c_str()]->SetDirectory(file1);
        for(int j=0; j<(int)branch.size();j++){
            branch1=branch[j];
            branchname=name+branch1;
            type=branchname+"/"+"D";
            TreeDouVar[branchname.c_str()]["Value"]=0.;
            Tree[name.c_str()]->Branch(branchname.c_str(),&TreeDouVar[branchname]["Value"],type.c_str());
